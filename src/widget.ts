...
// CSS is injected by Vite's Tailwind plugin — it gets bundled inline
// For Shadow DOM we inject it into the shadow root
...
class AvoqadoBookingWidget extends HTMLElement {
  ...
  private getProps(): WidgetProps {
    return {
      ...
      locale: (this.getAttribute('locale') ?? 'es') as 'en' | 'es',
      theme: (this.getAttribute('theme') ?? 'auto') as 'light' | 'dark' | 'auto',
